...
import { draftToExerciseDetailInput, resolveDefaultMemberId } from "./health-helpers";
import { emptyExerciseDetailDraft, type ExerciseDetailDraft } from "./health-types";

/**
 * Fast path for the common case — one activity, logged right now. No title, no
 * duration/ongoing, no type picker; timestamp is "now". See HealthEventSheet for backdating.
 */
export function LogExerciseSheet({
  ...
  const detail = draftToExerciseDetailInput(draft);

  async function save() {
    if (!detail) {
      setErr("Enter an activity.");
      return;
    }
    setBusy(true);
    setErr(null);
  ...
  return (
    <Sheet open={open} onClose={onClose} title="Log exercise">
      <fieldset className="space-y-4 px-6 py-4">
        ...
        <ExerciseDetailsEditor draft={draft} onChange={setDraft} />
        <label className="block space-y-1 text-sm">
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button onClick={() => void save()} disabled={busy || !detail}>
            Save
          </Button>
        </div>
      </fieldset>
