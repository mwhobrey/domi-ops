...
export function DriveObjectPicker({
  ...
  const fetchObjects = useCallback(async (q: string, tag: string | null, pinned: boolean) => {
    setLoading(true);
    setError(null);
    try {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      void fetchObjects(searchQuery, activeTag, pinnedOnly);
    }, 300);
    return () => clearTimeout(timer);
  }, [open, searchQuery, activeTag, pinnedOnly, fetchObjects]);
