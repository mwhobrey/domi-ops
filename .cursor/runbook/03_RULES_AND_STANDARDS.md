...
3. **Three Google OAuth flows** — login via Better Auth (`/auth/callback/google`), calendar (`/auth/google/calendar/*`), and Docs export (`/auth/google/docs/*`); all redirect URIs must be in Google Cloud Console (`docs/GOOGLE_OAUTH_SETUP.md`). Enable **Google Docs API** + **Google Drive API** for export.
