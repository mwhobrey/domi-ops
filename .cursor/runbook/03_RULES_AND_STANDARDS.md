...
2. **localhost vs 127.0.0.1** — `PUBLIC_APP_URL` / Google redirect URIs use one loopback host (usually `localhost`). Cookies set on `127.0.0.1` are not sent on `localhost` callbacks → Better Auth `state_security_mismatch`. Middleware + auth proxy redirect `127.0.0.1` → canonical host using the **`Host` header** (Next.js may normalize `request.nextUrl.hostname` to `localhost` while cookies stay on `127.0.0.1`). Browse `http://localhost:3000` (or `:3001` docker).
2b. **GHCR web image + OAuth** — CI builds the web image without your droplet `PUBLIC_APP_URL`. Do **not** pass `baseURL` to `createAuthClient` — better-auth defaults to `window.location.origin` in the browser. API/worker still read runtime `PUBLIC_APP_URL` from `.env` for Better Auth `baseURL` and Google redirect URIs.
3. **Three Google OAuth flows** — login via Better Auth (`/auth/callback/google`), calendar (`/auth/google/calendar/*`), and Docs/Drive (`/auth/google/docs/*`); all redirect URIs must be in Google Cloud Console (`docs/GOOGLE_OAUTH_SETUP.md`). Docs connect uses **`drive.file` only** (WHO-228; Family Link–friendly). Enable **Google Drive API**; exports convert via Drive upload (no Docs REST `documents` scope).
...
19. **Server clocks are UTC — "today" is the household's** (WHO-327). In the API, never derive a calendar date from `new Date().toISOString().slice(0, 10)` or `getMonth()`: after 7 PM Central that's already tomorrow. Use `householdTodayIsoDate` / `householdMonthKey` / `householdTimezone` (`apps/api/src/lib/household-time.ts`) and `localDateOfInstant` / `zonedLocalToUtc` from `@domi-ops/calendar-sync`. In web Server Components, pass an explicit `timeZone` to `toLocale*` (fetch it from `/api/core/household/settings`). In client components, "today" is `useHouseholdToday()` (`components/HouseholdTimeProvider.tsx`, fed by `householdTimezone` on `/auth/session` via `AppShell`), not `formatDateLocal(new Date())`: a member whose device is in another zone must still see the household's day. It's a React context on purpose; a module-level zone would be shared across requests during SSR. Timestamps ("5:23 PM") stay in the viewer's own zone via `LocalDateTime`. **Don't read the zone from `GET /api/core/household/settings` in a page any member can open:** it's admin-only (403), and pages that `.catch()` it fall back to `"UTC"` for everyone else (WHO-340 found this on `/health`).
20. **Never rebuild a DTO with a field whitelist the editor doesn't know about** (WHO-336). `CalendarPageClient.mapLoadedEvent` dropped `driveBuffer*`, `location`, and `attendeeMemberIds`. The sheet then read them as empty and saved `null`, silently wiping drive buffers. When adding a field to an editable DTO, grep for mappers like this.
21. **`Combobox.onQueryChange` must not re-fire on identity** (WHO-336). Callers pass inline arrows; keying an effect on the callback looped (~120 requests on `/expenses`). The component holds it in a ref and re-queries only when `value` changes.
