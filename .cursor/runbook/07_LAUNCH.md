...
**Goal:** real signal from one outside tester (Kort) going through the actual marketing → checkout → onboarding path, before committing to the public repo flip.
...
- **Live** Stripe keys/products/prices, with a 100%-off promotion code for Kort as a thank-you (`allow_promotion_codes` on Checkout).
...
**Must-do before handoff to Kort:**
...
- [ ] Provision the real shared instance — DO Managed Postgres, DO Spaces, droplet, Caddy, DNS. Checklist: [deploy/HOSTED_BETA_SETUP.md](../../deploy/HOSTED_BETA_SETUP.md) §1–5. (Operator-run, needs Mike's DO/Stripe account access.)
...
- [x] Testbook for Kort: [docs/marketing/KORT_TESTBOOK.md](../../docs/marketing/KORT_TESTBOOK.md).
